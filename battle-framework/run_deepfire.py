...
from agent.deepfire import RedAgent, BlueAgent
from agent.deepfire.params import run_params

from agent.deepfire.utils import LX15_all_die
        ...
        for i in range(num_episodes):
            ...
            while True:
                try:
                    ...
                    if done[0] or LX15_all_die(observation):  # 对战结束后环境重置
                        # 统计胜负结果
                    ...
                    if not str(e) == "result expired":
                        print(e, type(e), str(e), str(e).join(["'","'"]))
                        assert 0
            ...
            # Save the reward when episode is over.
            [agent for agent in self.agents if agent.side == 'red'][0].red_agent_deepfire.save_reward(i_episode=(i+1))
        # 关闭文件
        f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_id', type=int, required=False, default=int(run_params.env), help='id of environment')
