...
'''
consider the DA of COMMAND, with new function:
    ...
    def step(self, obs_red, obs_blue):
        ...
        for k in self.blue_units:
            ...
            if death > 0:
                print("blue:", k, death)
                if k == "11":
                    reward += 0.05 * death * 2.0  # Red side has about 2 times more LX 11 than blue side.
                if k == "12":
                    reward += 1.0 * death
                if k == "15":
                    reward += 0.0625 * death * 2.0  # Red side has about 2 times more LX 15 than blue side.
                if k == "21":
                    reward += 0.5 * death
                if k == "32":
                    reward += 1.0 * death
                if k == "41":
                    reward += 3.0 * death * 0.25  # for there need 4 missile for the death, the last one make death.
