...
# step action
from .lstm_policy import Red_agent_deepfire
from .cmd_check import Cmd_validate
from .utils import print_2, south_COMMAND_dead
...
class RedAgent(Agent):
    def __init__(self, name, config, **kwargs):
        ...
        # step action
        self.red_agent_deepfire = Red_agent_deepfire()
    ...
    def _init(self):
        ...
        # step action
        self.red_agent_deepfire.set_nn_model()
    ...
    def step(self, sim_time, obs_red, **kwargs):
        ...
        # print("")
        # print("="*30)
        # print(str(sim_time)+"-"+str(self.i_epsilon)+"-"+str(self.i_step))
        # print("")
        # with open("data/observation-json/observation-"+str(sim_time)+"-"+str(self.i_epsilon)+"-"+str(self.i_step)+".json","w") as f:
        #     f.write(json.dumps(observation))

        if not train_params.submit == 1:
            with open("./agent/deepfire/data/observation-json/observation-present.json","w") as f:
                f.write(json.dumps(observation))
        ...
        if self.agent_state == 0:
            '''
                script actions with rule agent.
            '''
            cmd_list = []
        ...
        elif not self.agent_state == 0:
            '''
                step action with networks agent.
            '''
            cmd = self.red_agent_deepfire.step(obs_red, observation, self.done, self.i_epsilon, sim_time)
            ...
            if sim_time > 30*60:
                fly_num = 2
                if obs_red["airports"][0]["BOM"] >= fly_num:
                    cmd.extend(self._takeoff_areapatrol(fly_num, 15, AREA_HUNT_POINT1, AREA_PATROL_PARAMS))
                    print('轰炸机HA起飞')

                fly_num = 1
                if obs_red["airports"][0]["JAM"] >= fly_num:
                    # 空中干扰南部干扰阵位

                    cmd.extend(self._takeoff_areapatrol(fly_num, 13, [-131154+30000, -87888+80000, DISTURB_PATROL_HEIGHT], DISTURB_PATROL_PARAMS))
                    print('干扰机起飞')
                    self.disturb_state = RedAgentState.AIR_DISTURB2

                if self.disturb_state == RedAgentState.AIR_DISTURB2:
                    for unit in obs_red['units']:
                        if unit["LX"] == 13 and unit["X"] < 50e3:
                            self.disturb_state = RedAgentState.AIR_DISTURB3

                            # 空中干扰南部干扰阵位
                            AIR_DISTURB_POINT4 = [-45000-40000, -55000+50000, DISTURB_PATROL_HEIGHT]

                            cmd.extend(
                                self._disturb_patrol(unit["ID"], AIR_DISTURB_POINT4, DISTURB_PATROL_PARAMS))

                # south COMMAND dead
                if south_COMMAND_dead(obs_red) and self.disturb_state == RedAgentState.AIR_DISTURB3:
                    for unit in obs_red['units']:
                        if unit["LX"] == 13:
                            self.disturb_state = RedAgentState.AIR_DISTURB4

                            # cmd.extend(self._disturb_linepatrol(unit["ID"], NORTH_LINE))

                            cmd.extend(
                                self._disturb_patrol(unit["ID"], AIR_DISTURB_POINT2, DISTURB_PATROL_PARAMS))

            fly_num = 2
            if obs_red["airports"][0]["AIR"] >= fly_num:
                cmd.extend(self._takeoff_areapatrol(fly_num, 11, AIR_PATROL_POINT3, AIR_PATROL_PARAMS))
                print('阻援歼击机JE起飞')



            '''
                only one a2a executes airattack.
                if the enemy in blue_list is being attacked.
                don`t need add another a2a to attack it.
            '''
            assert self.blue_list == []
            for a2a in obs_red['units']:
                # 此时对状态为15 或 13 的 red side LX=11 飞机进行判断
                if a2a['LX'] == 11:
                    if a2a['ST'] == 15 or a2a['ST'] == 13:
                        assert a2a["ID"] in self.blue_dic
                        self.blue_list.append( self.blue_dic[ a2a["ID"] ] )
            ...
            for blue_unit in obs_red['qb']:
                ...
                if blue_unit['LX'] == 11 or blue_unit['LX'] == 15 or blue_unit['LX'] == 12:
                    if blue_unit['WH'] == 1 and blue_unit['ID'] not in self.blue_list:
                        ...
                        dic_distance = {}
                        for a2a in obs_red['units']:
                            if a2a['LX'] == 11 and a2a['Fuel'] > 3000 and '170' in list(a2a['WP'].keys()) and int(
                                    a2a['WP']['170']) > 0:
                                distance = math.sqrt(
            ...
            # check with the bottom line.
            try:
                if self.cmd_validate._action_validate(cmd, obs_red, "red", sim_time)=="allowed":

                    print("")
                    print("cmd check allowed!")
                    print("")
                ...
                print("")
                print("-- ERROR --"*10)
                print("cmd check with exception!")
                print("cmd:", cmd)
                print("Exception:", e)
                print("")
                return []
        ...
        if self.a2g_ha == 0 and 'HA' in list(self.team_id_dic.keys()):
            for a2g in obs_red['units']:
                if a2g['TMID'] == self.team_id_dic['HA'] and -75000 < a2g['X'] < -35000 and 50000 < a2g['Y'] < 80000:
                    for a2a in obs_red['units']:
        ...
        if self.a2g_hb == 0 and 'HB' in list(self.team_id_dic.keys()):
            for a2g in obs_red['units']:
                if a2g['TMID'] == self.team_id_dic['HB'] and -75000 < a2g['X'] < -35000 and 50000 < a2g['Y'] < 80000:
                    for a2a in obs_red['units']:
